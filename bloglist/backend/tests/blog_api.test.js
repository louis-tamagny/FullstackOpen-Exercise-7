...
describe('new blog is created', () => {
  test('valid', async () => {
    ...
    expect(response.body[response.body.length - 1].author).toEqual(
      newBlog.author
    )
...
describe('blog is deleted', () => {
  ...
  test('invalid delete', async () => {
    ...
    let result = await api
      ...
      .set('Authorization', authorization)
      .send(newBlog)
    await api.delete(`/api/blogs/${result.body.id}`).expect(401)
  })
})
...
test('a blog is updated', async () => {
  ...
  await api.put(`/api/blogs/${response.body[0].id}`).send(newBlog).expect(200)
